...
# Configure Maven Mirror url
configure_maven_mirror(){
 if [ -n "$MAVEN_MIRROR_URL" ]; then
    echo "Configuring Maven Mirror to ${MAVEN_MIRROR_URL}"
    xml="    <mirror>\
...
startup() {
  ...
  if [ -n "${JAVA_MAIN_CLASS:-}" ] ; then
     ...
     args="-jar ${JAVA_APP_JAR} --thin.name=${THIN_NAME} --function.name=${FUNCTION_NAME} --spring.main.sources=${JAVA_APP_NAME}"
  fi
  ...
  echo exec $(exec_args) java $(java_options) ${args} $*
  # exec $(exec_args) java $(java_options) -cp "$(classpath)" ${args} $*
  exec $(exec_args) java $(java_options) ${args} $*
}
